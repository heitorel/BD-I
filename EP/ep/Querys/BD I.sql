...
CREATE TABLE CONFLITO (       
    ...
    ID_Grupo INT NOT NULL,  
    flag_territorial int not null,
	flag_religioso int not null,
	flag_economico int not null,
	flag_racial int not null,
...
ALTER TABLE CONFLITO ADD CONSTRAINT tipos_conflitos check (flag_economico + flag_racial + flag_religioso + flag_territorial = 1);

DELIMITER $

CREATE TRIGGER integridade_baixas AFTER  UPDATE 
ON divisao
FOR EACH ROW
BEGIN
	UPDATE GRUPO_ARMADO SET baixas_totais = baixas_totais + NEW.baixas
WHERE ID_Grupo_Armado = NEW.ID_Grupo_Armado;
END$

CREATE TRIGGER sequencialidade_divisao_update AFTER  UPDATE 
ON divisao
FOR EACH ROW
BEGIN
	UPDATE GRUPO_ARMADO SET ID_Divisao = ID_Divisao + NEW.ID_Divisao
WHERE ID_Grupo_Armado = NEW.ID_Grupo_Armado;
END$

CREATE TRIGGER sequencialidade_divisao_insert AFTER  INSERT 
ON divisao
FOR EACH ROW
BEGIN
	UPDATE GRUPO_ARMADO SET ID_Divisao = ID_Divisao + NEW.ID_Divisao
